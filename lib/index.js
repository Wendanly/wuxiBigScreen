 ...
 axios.interceptors.request.use(function (config) {
   // 在发送请求之前做些什么
   config.data = paramsInfo.params;
   config.url += `?token=${paramsInfo.token}`;
   return config;
 }, function (error) {
   // 对请求错误做些什么
 ...
 Vue.component('my-box', {
   ...
   data() {
     return {
       ...
       sufixIcon: './img/jiantou.png',
       titleBorder: './img/xiantiao.png',
       bTitleBorder: './img/daxiantiao.png',
       vertialList: [],
       sexList: [],
     }
   },
 ...
 new Vue({
   ...
   methods: {
     init() {
       paramsInfo = getBP(); //获取入参和token
       this.getWea(); //先调本地服务，获取基本信息（天气、logo、title）
       // 获取人数
       this.getRealCust();
       //图1~图7
       this.request1();
     ...
     getWea() {
       axios.post('getBaseInfo').then(res => {
         if (res.data.state == 0) {
     ...
     //  实时客流、实时累计客流 ,展会累计人数
     getRealCust() {
       axios.post('getRealCust').then(res => {
         ...
         resp.map((o, j) => {
           this.boxList[j].numList = String(o.num).padStart(String(o.num).length, '0').split('');
         });
       });;
     },
     //  省内
     request1() {
       axios.post('getProvinceCustSrc').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     //  国内
     request2() {
       axios.post('getCountryCustSrc').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     //  国际
     request3() {
       axios.post('getInternationalSrc').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     //  APP排名
     request4() {
       axios.post('getAppRank').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     //  性别比例
     request5() {
       axios.post('getSexRatio').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let result = {
           dim: ['男', '女'],
     ...
     //  年龄分布
     request6() {
       axios.post('ageStratification').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     //  客流趋势
     request7() {
       axios.post('getCustTrend').then(res => {
         if (res.data.state !== 0) return this.$message.warning(res.data.msg);
         let obj = res.data.obj;
         let dim = [],
           target = [];
     ...
     char6(dom, seriesData, xAxisData) {
       ...
       let data = [];
       //  seriesData = [20, 25, 26, 50, 40, 16, 46, 15];
       //  xAxisData = ['0-15岁', '16-17岁', '18-24岁', '25-34岁', '35-44岁', '45-59岁', '60-74岁', '>=75岁'];
